...
var (
	...
	// or minted too long ago: a bad signature, another issuer, another audience, or
	// an expiry outside whatever window the caller allowed. One error for all of
	// them, since a caller can do nothing differently in any of the cases.
...
// nothing else, so that a caller allowed to renew an expired token can tell it
// apart from a token that was never ours. Treating it like any other error refuses
// the token, which is the reading every other caller should take.
