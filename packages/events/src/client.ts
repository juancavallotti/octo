...
/** The conventional route a host mounts {@link integrationEventStream} on. */
export const INTEGRATION_EVENTS_PATH = "/api/integrations/events";

/**
 * Subscribe to the event stream from the browser: opens an EventSource to the events
 * route, parses each frame as an {@link OctoEvent}, and invokes `onEvent`. Returns a
 * function that closes the connection. Browser-only (uses EventSource).
 */
