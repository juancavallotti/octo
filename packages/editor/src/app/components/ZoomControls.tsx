...
export default function ZoomControls({ onFit }: { onFit: () => void }) {
  const { zoom, zoomIn, zoomOut, reset, dragging } = useCanvasZoom();
  ...
  return (
    <div className="absolute bottom-4 right-4 z-30 flex items-center gap-0.5 rounded-lg border border-black/10 bg-white/90 p-0.5 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/90">
      <Button label="Zoom out" onClick={zoomOut} disabled={dragging || zoom <= ZOOM_MIN}>
        <Minus size={14} />
      </Button>
      ...
      <button
        ...
        className="min-w-12 rounded px-1 py-1 disabled:opacity-40 text-center font-mono text-xs tabular-nums text-zinc-500 transition-colors hover:bg-black/[0.06] hover:text-zinc-800 dark:text-zinc-400 dark:hover:bg-white/[0.08] dark:hover:text-zinc-100"
      >
        {Math.round(zoom * 100)}%
      </button>

      <Button label="Zoom in" onClick={zoomIn} disabled={dragging || zoom >= ZOOM_MAX}>
        <Plus size={14} />
      </Button>

      <Button label="Fit flows in view" onClick={onFit} disabled={dragging}>
        <Maximize2 size={13} />
      </Button>
    </div>
  );
