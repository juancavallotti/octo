...
/**
 * The Pinecone brand mark. lucide ships no brand icons, so this is the
 * canonical Pinecone logomark inlined as a single monochrome SVG path (the
 * icon Pinecone renders next to its wordmark, cropped to its own viewBox). It
 * is typed to match `LucideIcon` (accepting `size`/`className` and forwarding
 * a ref) so it drops straight into the icon registry alongside the lucide
 * set. The mark is monochrome, so it fills with `currentColor` and adapts to
 * the editor theme.
 */
export const PineconeIcon = forwardRef<SVGSVGElement, LucideProps>(
