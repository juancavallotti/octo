...
const AI_AGENT_MEMORY: Example = {
  ...
  summary:
    "An ai-agent with memoryThreadId loads a thread's prior transcript before its run and saves it after, so conversations persist across invocations; contextMaxTokens + memoryCompaction (prune|summarize) bound it. Add agentId to opt into first-class memory: conversation history recorded UNCOMPACTED and listable by the platform, working memory checkpointed during the run, and — with userId + userMemory — remember/forget/search_memory tools for curated facts about a person. agentId is stated, never derived: a name taken from the block's position would lose every conversation when the block is renamed. Add a nameThread slot to have the agent name its own conversations — it runs once on the opening exchange, is handed {question, answer}, and whatever string (or {\"title\": \"...\"}) it returns becomes the title; the engine does the writing. Without userId a conversation is stored attributed to nobody and will not appear in anything that lists by person. When input folds in context the person did not say (the page they are on, a catalogue the model needs), set question to the raw message: input is what the model gets, question is what history records, and recording the whole prompt puts it in the transcript, the title and the embedding. The clear-agent-memory block wipes a thread (pass the same agentId to reach the durable copy). Needs ANTHROPIC_API_KEY. The agent's fields (connector/prompt/tools/agentId/userId/memory*) sit at the block top level.",
