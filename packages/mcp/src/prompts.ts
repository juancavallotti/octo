...
7. Call \`run_integration\` for a full run. If the integration serves HTTP at the address the runner injects (an \`http\` source whose connector leaves its address to the environment — see the tip below, and note that an HTTP source alone is not enough), the result includes a \`testUrl\` you can curl to exercise its endpoints; otherwise it runs internally (e.g. cron-driven). Read \`get_run_logs\` to see the runtime's own load errors, iterate with \`update_integration\`, and \`stop_integration\` when done.
...
Tips:
- To make an integration testable over HTTP, give a flow an \`http\` source and leave its connector's ADDRESS to the environment. The runner injects HTTP_PORT/HTTP_HOST and reaches the run at what it injected, so the connector must take them: either set no \`host\`/\`port\` at all (a configured \`http\` connector is optional — one is resolved on demand), or write \`port: \${HTTP_PORT}\` with HTTP_PORT declared under \`env\`. A pinned \`port: 9000\`, a \`port: \${OTHER_VAR}\`, or a pinned non-bind-all \`host\` all win over the environment, so the run serves somewhere the runner cannot reach and gets no test URL — as does declaring HTTP_PORT with no \`http\` source to read it. Two \`http\` connectors both left to the environment take the same injected port and the run dies on "address already in use"; give the second one a port of its own.
- Keep \`service.name\` stable; renaming may change the integration's id.
- CEL expressions (e.g. log messages, payloads) can read body, vars, eventID, and correlationID. On top of the CEL standard library they get octo's own functions (toJson/fromJson, toFormData/fromFormData, toYaml/fromYaml, toEnv/fromEnv, templateResource, multipart/addPart/fromMultipart/toMultipart for file uploads, and hmacSha256/hmacSha1/hexEncode/secureCompare for verifying a signed webhook) plus the cel-go utility libraries — strings (upperAscii, trim, split, join, format), lists (distinct, sort, sortBy, flatten, slice), encoders (base64), math, two-variable comprehensions (transformMap, transformMapEntry), sets, regex, and optional (orValue, hasValue, first/last, optional.of/none/unwrap). Reach for those before adding a \`foreach\`: most reshaping is one expression. Call \`getCelFunctions\` for the exact catalogue, or \`getCelFunctions\` with a \`library\` for one family (request \`optional\` when using regex.extract or optional chaining).${docs}`;
}
