...
- CEL expressions (e.g. log messages, payloads) can read body, vars, eventID, and correlationID. On top of the CEL standard library they get octo's own functions (toJson/fromJson, toFormData/fromFormData, templateResource) plus the cel-go utility libraries — strings (upperAscii, trim, split, join, format), lists (distinct, sort, sortBy, flatten, slice), encoders (base64), math, two-variable comprehensions (transformMap, transformMapEntry), sets, regex, and optional (orValue, hasValue, first/last, optional.of/none/unwrap). Reach for those before adding a \`foreach\`: most reshaping is one expression. Call \`getCelFunctions\` for the exact catalogue, or \`getCelFunctions\` with a \`library\` for one family (request \`optional\` when using regex.extract or optional chaining).${docs}`;
}
