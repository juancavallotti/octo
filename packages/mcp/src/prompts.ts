...
7. Call \`run_integration\` for a full run. If the integration serves HTTP (any flow whose source is an \`http\` source — declaring HTTP_PORT is optional, the connector reads it from the environment the runner supplies), the result includes a \`testUrl\` you can curl to exercise its endpoints; otherwise it runs internally (e.g. cron-driven). Read \`get_run_logs\` to see the runtime's own load errors, iterate with \`update_integration\`, and \`stop_integration\` when done.
