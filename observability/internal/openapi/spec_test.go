...
// two tags, a schema that references another, and a schema nothing points at.
const sample = `{
  "openapi": "3.1.0",
  "paths": {
    "/logs": {
      "get": {
        ...
        "responses": {"200": {"content": {"application/json": {"schema": {"$ref": "#/components/schemas/LogPage"}}}}}
      }
    },
    "/traces": {
      "get": {
        ...
        "responses": {"200": {"content": {"application/json": {"schema": {"items": {"$ref": "#/components/schemas/Trace"}, "type": "array"}}}}}
      }
    },
    "/traces/{traceId}": {
      "get": {
        "tags": ["traces"],
        "responses": {"200": {"content": {"application/json": {"schema": {"oneOf": [{"type": "object"}, {"$ref": "#/components/schemas/TraceDetail"}]}}}}}
      }
    }
  },
  "components": {
    "schemas": {
      "LogPage": {"type": "object", "properties": {"items": {"$ref": "#/components/schemas/LogRow"}}},
      "LogRow": {"type": "object"},
      "Trace": {"type": "object"},
      "TraceDetail": {"type": "object"},
...
func TestFilterByTagKeepsOnlyThatTag(t *testing.T) {
	out, err := Filter([]byte(sample), "logs", "")
	if err != nil {
	...
	if _, ok := paths["/logs"]; !ok {
		t.Errorf("want /logs kept, got %v", paths)
	}
}
...
func TestFilterByPathIsAPrefixMatch(t *testing.T) {
	out, err := Filter([]byte(sample), "", "/traces")
	if err != nil {
		t.Fatalf("Filter: %v", err)
	}
	paths, _ := decode(t, out)
	...
	if _, ok := paths["/traces/{traceId}"]; !ok {
...
// every schema the service stores.
func TestFilterPrunesUnreachableSchemas(t *testing.T) {
	...
	if _, ok := schemas["LogPage"]; !ok {
		t.Error("want the referenced LogPage kept")
	}
	for _, name := range []string{"Trace", "TraceDetail", "Unreferenced"} {
		if _, ok := schemas[name]; ok {
			t.Errorf("want %s pruned, it is unreachable from the logs tag", name)
		}
	}
}

// Reachability is transitive: LogPage references LogRow, and a caller handed
// LogPage without LogRow has a document it cannot resolve.
func TestFilterFollowsReferencesThroughSchemas(t *testing.T) {
	...
	for _, name := range []string{"LogPage", "LogRow"} {
		if _, ok := schemas[name]; !ok {
			t.Errorf("want %s kept, it is reachable from the logs tag", name)
		}
	}
...
// The generator nests schemas inside oneOf and content types, so a walker that only
// looked where a schema is "supposed" to be would miss TraceDetail entirely.
func TestFilterFindsReferencesInsideOneOf(t *testing.T) {
	out, err := Filter([]byte(sample), "", "/traces/{traceId}")
	if err != nil {
		t.Fatalf("Filter: %v", err)
	}
	_, schemas := decode(t, out)

	if _, ok := schemas["TraceDetail"]; !ok {
		t.Error("want the response schema found inside oneOf")
	}
}
...
func TestEmbeddedSpecDescribesRoutes(t *testing.T) {
	...
	if len(paths) == 0 {
		t.Fatal("the embedded spec describes no routes; regenerate with `task observability:openapi`")
	}
	if _, ok := paths["/logs"]; !ok {
		t.Errorf("want /logs in the embedded spec, got %d paths", len(paths))
	}
}
