...
const (
	...
	// slice of the catalogue at one model's rate.
	minSlugLength = 4
)
...
//
// The feed carries much this does not name, including per-image and per-request
// charges: a record reports token counts and nothing else, so there is no quantity
// to multiply those by. A model billed per call or per image is under-costed by
// that portion unless the provider reported a cost of its own.
...
// gpt-4o-2024-11-20 from the longer one. The second return counts entries that
// could not become a rate, so a feed that stops pricing things says so.
func openRouterRows(models []openRouterModel) (rows []catalogueRow, skipped int) {
	...
	for _, m := range models {
		...
		// The last check is the one that matters: a model the feed prices at nothing
		// is a model it has NOT priced -- the auto-router publishes "-1" for every
		// field -- and admitting it would price every call it matches at free.
...
// carrying no provider of its own, so a row for anthropic/claude-sonnet-4.5 has to
// say ANTHROPIC to get the exclusive cached-token rule. A record that names
// OPENROUTER itself wins, since those counts follow the inclusive convention
// whatever vendor served them.
...
//
// The check is on the CONVERTED figure rather than the parsed one, which catches
// the case the parsed check cannot see: a value finite on its own that overflows
// to infinity once scaled by a million.
...
// rather than zero when a model publishes no cache price. Nil decides whether
// cached tokens can be priced at all, so an absent field stays absent all the way
// to the rate.
//
// A published figure that is not a price is treated as absent, which keeps a model
// with one bad cache half priceable at priced_partial.
