...
// Repo persists the embedding settings row. It owns the key and the marshalling;
...
func (r *Repo) Get(ctx context.Context) (stored, error) {
	...
	if err := json.Unmarshal(raw, &s); err != nil {
		return stored{}, fmt.Errorf("embedding settings: decode stored value: %w", err)
	}
	return s, nil
}
	...
	return r.store.Mutate(ctx, settingsKey, func(raw json.RawMessage, ok bool) (json.RawMessage, error) {
		...
		if ok {
			if err := json.Unmarshal(raw, &cur); err != nil {
				return nil, fmt.Errorf("embedding settings: decode stored value: %w", err)
			}
		}
		next, err := fn(cur)
		if err != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("embedding settings: encode value: %w", err)
		}
		return out, nil
	})
}
