...
func (s *Service) Deploy(ctx context.Context, integrationID string, settings Settings) (Deployment, error) {
	...
	// the deployed definition: its HTTP_PORT declaration, or the port the http
	// connector backing its sources would bind. A definition with an HTTP source is
	// "networked" and gets a Service, a unique internal slug/URL and the option of
	// external exposure. Anything else (a timer, a scheduled job) runs as a bare
	// workload.
...
func (s *Service) resolveEnvBindings(ctx context.Context, runtimeEnv map[string]string, bindings map[string]EnvBinding) (literal, secret map[string]string, err error) {
	...
	for name, b := range bindings {
		if name == envHTTPPort || name == envHTTPHost || name == envObservabilityURL {
