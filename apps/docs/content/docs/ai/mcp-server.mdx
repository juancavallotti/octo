...
The router is a protocol adapter and calls no LLM. The intelligence lives in
...
[`ai-agent` tool](/ai/agent-skills-and-tools). `name` is the tool name clients
...
| `resource` | The alias of a [template resource](/concepts/resources) declared under `resources.templates`. The template is rendered on each read. |
...
Then ask Claude Code to fetch a forecast; it discovers the `forecast` tool via
