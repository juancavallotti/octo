...
**Free is the default.** A panel that asks about every read trains a person to
click yes without reading. Gate the call that changes something.
...
`input` is the arguments **as the model asked for them**: the person is
...
(see [Steering a running agent](/ai/agent-signals)).
...
A run parked on a tool call is billed for the wait, so every way of not being
allowed ends the same: the call is **denied**, and the denial is the tool's
result.
...
gets a turn to say so, ask, or take another route. Like any tool result it
survives compaction and is in the transcript afterwards.
...
A block that gates anything must have a way to ask and a way to be told. Each
missing half fails the build rather than turning up as a denial five minutes
into a run.
...
`authorize` is an `ai-agent` setting. An [`mcp-router`](/ai/mcp-server) rejects
