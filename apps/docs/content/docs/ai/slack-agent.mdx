...
The memory settings give each thread a persistent transcript, budgeted to
6,000 tokens and **summarized** (rather than pruned) when it overflows, so
long conversations keep their gist. See [Agent Memory](/ai/agent-memory) for
the mechanics.
