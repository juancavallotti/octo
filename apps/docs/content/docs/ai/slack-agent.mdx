...
- **Budget memory deliberately** — `contextMaxTokens` caps the size of the
  prompt, not the bill. `summarize` compaction keeps the gist of a long thread
  rather than its opening turns, and buys that with a model call of its own.
