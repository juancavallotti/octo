...
| KV / object store | Serialized to `--storage-dir`; volatile namespaces and secrets in memory | Orchestrator HTTP KV (persistent, encrypted secrets); Redis, dialed directly, for volatile namespaces |
