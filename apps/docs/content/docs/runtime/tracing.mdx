...
| k8s (`-tags k8s`) | The `internal.traces` NATS subject, consumed by the platform's aggregator, which stores every record and prices any model calls — see [Traces](/platform/traces). |
