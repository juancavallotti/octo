...
to *this* message". [Metrics](/runtime/observability) are aggregates whose labels
all come from the config and never from message data, so they can tell you a flow
is failing but never which message failed. [Spies](/reference/cli#spies) record
...
octo invoke --config app.yaml --flow orders --data '{"orderId":"A-1"}' \
  --traces --traces-file /tmp/orders.jsonl
```

And `dolphin` will run a whole suite that way, one file per case:

```bash
dolphin test orders_test.yaml --traces-dir ./traces
```

That is the cheapest way to find out what a flow's messages actually look like:
...
and one synchronous, is one trace:
...
[`turn_end` event](/reference/blocks/ai#agent-events) reports, so the two observers
...
fills, records are **dropped rather than blocking the flow**, because tracing must
never put a flow worker behind a disk write.
...
| k8s (`-tags k8s`) | The `internal.traces` NATS subject, consumed by the platform's observability service, which stores the records and prices the model calls among them. See [Traces](/platform/traces). |
