...
<Callout type="info">
Flow lifecycle events (`started`/`completed`/`dropped`/`failed`) stay on a process-local bus (`core.EventBus`), and block events on a process-local dispatcher (`core.BlockEvents`) — two separate mechanisms, neither of which leaves the process. Nothing is shipped to a central trace or metrics store, and Octo exposes no metrics endpoint of its own. To export either, embed the runtime and subscribe (see below).
</Callout>
