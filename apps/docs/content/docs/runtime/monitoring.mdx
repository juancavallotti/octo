...
<Callout type="info">
Flow lifecycle events (`started`/`completed`/`dropped`/`failed`) and block events stay on a process-local bus — nothing is shipped to a central trace or metrics store, and Octo exposes no metrics endpoint of its own. To export metrics, embed the runtime and subscribe (see below).
</Callout>
...
Two blocks that address ambiguously (two unnamed blocks of the same type in one chain) share a path. Name a block to give it one of its own.
...
**Sync** — for debugging. Listeners run inline on the flow's own goroutine, before the block runs, and the flow waits for them:
