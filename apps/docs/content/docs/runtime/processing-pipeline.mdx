...
`failed` means **unhandled**. A message that failed but whose flow-level error path recovered it is published as `completed`, because that is what happened to it — so anything counting `failed` counts what nothing handled, not everything that went wrong.
