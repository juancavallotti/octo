...
<Callout type="warn">
  A block that clones — `flow-ref`, `queue-dispatch`, `publish-event` — copies the *whole* body every time it runs. Inside a `foreach mode: map` body, that body is the entire collection, so the loop is quadratic again no matter how cheap the scope was. Put a `set-payload` before such a block to narrow the message to just what the sub-flow needs.
</Callout>
