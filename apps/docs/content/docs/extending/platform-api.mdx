...
The full contract is an OpenAPI 3.1 document. Start by reading the copy published
here — **[octo-platform-api.yaml](/octo-platform-api.yaml)** — which is what to
use while you are still deciding whether to implement it.
...
version-matched to the thing that will call your server rather than to whatever
this website last published:
...
<Callout type="info">
`octo openapi` exists only in the `-tags api` build, alongside the provider that speaks the contract — the standalone and cluster binaries carry neither. Read the YAML rather than the JSON: the comments are half the document, they say why each route is shaped the way it is, and the JSON rendering drops them.
</Callout>
