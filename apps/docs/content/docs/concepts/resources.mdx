...
A span is ordinary CEL in the same environment as any other expression, so every [custom function](/reference/cel/octo-extensions#functions) works inside one — including `templateResource`, which lets a template pull in another:
...
Nesting is capped at 8 levels, so a template that renders itself fails with a clear error rather than recursing without end.
