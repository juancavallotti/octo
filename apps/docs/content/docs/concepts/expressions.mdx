...
This page covers where expressions live; the [CEL reference](/reference/cel) tours the language itself — operators, macros like `has()` and `map()`, type conversions, and the custom functions Octo adds (such as `templateResource` and `fromFormData`).
