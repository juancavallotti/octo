...
Signing in is how anybody reaches the editor — there is no open mode, and the
chart refuses to render without the settings below. Any provider that speaks the
...
<Callout type="warn">
  ...
  minted. See [MCP authentication](/ai/mcp-auth).
</Callout>
