...
to Artifact Registry. Two more are **public images** for use without a cluster:
the standalone editor and the runtime on its own.
...
The last row shares a Dockerfile with the cluster's `octo-runtime` but not a
binary: the public image is the default build, the cluster image is built with
`-tags k8s`. See [octo-runtime](#octo-runtime) below.
...
- **`compile`** (the default) — the Dockerfile builds the binary itself, in a
  `golang:1.25-alpine` stage. Nothing extra has to be passed, and the file works
...
it builds both providers: `prebuilt-k8s` and `prebuilt-standalone` instead of a
plain `prebuilt` (its `compile` arm still reads `GOTAGS`). One value rather than
two arguments, so there is no way to ask for the k8s image and get the standalone
binary.
