...
Octo ships six images. Five are the platform images, built together and pushed
to your own registry (Artifact Registry in the reference deployment); the
sixth is the public standalone editor on Docker Hub.
...
The standalone image is built by the GitHub Actions release workflow
(`.github/workflows/release.yml`) and pushed to public Docker Hub.
...
`octo` binary, built `CGO_ENABLED=0` with `-tags k8s` so it includes the
cluster services provider (Lease-based leader election, orchestrator-backed
KV, NATS queues). The final stage is
`gcr.io/distroless/static-debian12:nonroot` — no shell, no package manager,
runs as `nonroot`.

The default command is `octo run --config /etc/octo/integrations`. The
orchestrator deploys one Deployment per integration from this image, mounting
the integration's YAML into that directory via a per-deployment ConfigMap; the
Helm chart hands the image reference to the orchestrator as `RUNTIME_IMAGE`.
...
Unlike `octo-runtime`, its bundled binary is the default (untagged) build with
the standalone services provider. See
