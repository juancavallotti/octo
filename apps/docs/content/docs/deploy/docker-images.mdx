...
| `octo-orchestrator-paas`, `octo-logs-paas`, `octo-runtime-paas`, `octo-devruntime-paas`, `octo-devsidecar-paas`, `octo-statssidecar-paas` | `65532` (distroless `nonroot`) |
...
The pod stats sidecar, which runs beside a **deployed** integration rather than a dev
run. It samples that pod's own runtime metrics once a second and writes a rolling week
of them to Redis — see [Pod Stats](/docs/platform/pod-stats). Off by default; the chart
value is `orchestrator.podStats.enabled`.

Built from `sidecars/stats/Dockerfile` with the `sidecars` module as its context, the
same module and context as `octo-devsidecar`. Distroless `nonroot`. It holds **no**
Kubernetes credential and mounts no volume: it speaks to the runtime on the pod's own
loopback and to Redis, and to nothing else.
