...
deploying it to [Cloud Run](#deploy-to-cloud-run), and the
...
scan, but they are still readable as [resources](/concepts/resources), since the
...
`required: true` is **not** satisfied by a `default:`. The variable must come
...
Name the service account the revision runs as. Cloud Run's default is the Compute
...
Then let it invoke the callee. The binding is scoped to that one service, and
...
provider](/concepts/runtime-services) a local `octo run` uses: in-process queues,
...
For a durable cron, a shared KV, or queues that outlive an instance, there are
two ways to get them without giving up serverless.
...
crons, an orchestrator-backed KV and NATS-backed queues built in. See
...
App Runner can only pull from **Amazon ECR**, so push the image there first. This
is the one step that is not clickable:
...
The caveats travel with the runtime, not the platform: all of these scale to zero
or scale out, so the [stateless rule](#what-does-not-survive-scale-to-zero) holds
everywhere.
