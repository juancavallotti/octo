...
<Callout type="warn">
  ...
  kubeconfig holds cluster-admin credentials. Both are gitignored, so keep the
...
| `oidc_enabled` + `oidc_client_id` / `oidc_client_secret` / `oidc_issuer` | `false`, then unset | Gate the editor behind OIDC SSO; consumed by the `release` root. |
...
<Callout type="info">
  **Upgrading an installation created before this change.** The namespace used to
  ...
  The credentials come from the same state, so the apply replaces chart values
...
its version. See [Releases and upgrades](/deploy/releases-and-upgrades).
...
`release` root sets to your domain. See [Deployments](/platform/deployments) for
...
Postgres data lives on the boot disk (k3s `local-path`), so it survives reboots
