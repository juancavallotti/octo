...
<Callout type="warn">
  **Treat these roots as a worked example, not as your production infrastructure.**
  ...
  The chart is the stable interface here, not the Terraform: none of what these
  roots do to it (hostnames, TLS mode, external database, secrets) requires
  their Terraform.
</Callout>
...
Every pod is billed on its resource requests, and Autopilot injects defaults when
they are absent, so the profile sets them explicitly with limits equal to
requests: the Guaranteed QoS class, and a predictable bill.
...
Pods can take noticeably longer to start, because Autopilot may provision a node
first. The profile allows a generous startup probe (30 × 10s) so a slow first
boot is not killed before it is ready.
