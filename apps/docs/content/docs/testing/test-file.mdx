...
A case's mock can also be `null`, which **removes** the file's mock for that address so
the case runs the real block. See [un-mocking](#un-mocking-a-block-for-one-case).
...
A case can also go the other way and **lift** the file's mock, so that one case runs the
real block. Write the address with a `null`:
...
It is for the suite with one case whose whole point is what the real block does — the case
asserting that it fails loudly, or that it rejects its input. Without it, the file-level
`mocks:` has to be dropped and repeated on every other case, which is the duplication
file-level mocks exist to remove.

Only an address the file actually mocks may be nulled. Un-mocking anything else does
nothing at all, so the misspelled `progres.accepted: null` — one letter short of the mock
above — would read as *this case runs the real block* while the mock it meant to lift
stayed on: a case passing for the opposite of the reason it says. dolphin refuses the file
instead, naming the address.

<Callout type="info">
**`mocks: {}` is not an un-mock.** An empty map contributes nothing, so the file's mocks
all still apply — which is why the null exists rather than leaving people to discover
that the obvious spelling does nothing.
</Callout>
