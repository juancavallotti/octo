...
result becomes the body, or in `set-variable` when you want to keep the body and
hold the rendered text for a later block.

Parse on the way in, work with an ordinary body, and render on the way out — as
[`samples/text-formats.yaml`](https://github.com/juancavallotti/octo/blob/main/samples/text-formats.yaml)
does, reading a YAML service config and emitting its deployment settings as
`.env` content:
...
Expect two things when you write against the parsed body. Treat a YAML integer as
a number — `fromYaml` normalizes to the JSON-native shapes a body may hold, so a
YAML timestamp arrives as its RFC 3339 string too. And compare `fromEnv` values
as strings, with `body.PORT == "8080"`, or convert them with `int(body.PORT)`,
since an env file carries no types. See
