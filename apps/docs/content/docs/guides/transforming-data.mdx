...
`if` takes one `condition` and a `then` / `else` sub-flow. `foreach` evaluates
...
Prices are in integer cents because a JSON number is a binary double: a decimal
price cannot be held exactly, and rounding it back into shape loses a cent on
some values. See the [warning under Math](/reference/cel/libraries#math).

[Extension Libraries](/reference/cel/libraries) has the full vocabulary (strings,
