...
**Connectors.** Two `http-client` connectors give the agent its boundaries: `octo` for the orchestrator's API and `observability` for stored logs and traces. Their base URLs come from the two [platform access grants](/platform/deployments#platform-access), which inject `ORCHESTRATOR_URL` and `OBSERVABILITY_URL` into the pod. Tick both under **Advanced** when you deploy.
