...
<Callout type="info">
A file like this one, plus **what should have happened**, is a test. That is all
[`dolphin`](/testing/writing-cases) is: it writes a debug config per case, runs it
...
A debugging session already contains a test: the input, the mocks that made the flow
runnable, and what came back. Writing it down as a
[dolphin suite](/testing/writing-cases) is what stops the next person — or the next
you — from working it out again. The editor will do the writing: a run on the console
carries **Save as test case**, and the case then appears in that flow's ▶ menu as a
scenario to replay. See [Testing flows in the editor](/testing/in-the-editor).
