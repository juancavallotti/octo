...
```json
{"error":"block \"call-charge\": rest request: … connect: connection refused",
...
```json
{"error":"block \"call-charge\": card declined","failedBlock":"call-charge","flow":"charge-flowlevel"}
```
