...
A tool with no `authorize` runs freely. A panel that asks about every read
...
ordinary frame, so a caller already reading the run receives it:
...
A non-empty `authorizeId` makes a request an answer rather than a message: it is
...
<Callout type="warn">
**Whatever the thread id is derived from is exactly who can answer**, because an
...
That request returns `{}` immediately and the first terminal carries on. Send
...
It reaches the model as an error result, so the run continues. Like any other
tool result it survives compaction and is in the transcript afterwards.
...
[`rest-dynamic`](/reference/connectors/http) block instead. Is the danger in the
...
- [Agentic self-healing](/guides/agentic-self-healing) puts the gate on a Slack thread, answered by a reply, for an agent that triages platform alerts.
...
- [Dr. Octo](/platform/agent): the gate as a person meets it.
