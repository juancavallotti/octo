...
- **Matching is on the resolved path, not the string you typed.** A bare name
  matches the absolute path `$PATH` resolves it to, and `..` segments are
  normalized, so `/bin/../bin/sh` cannot dodge an entry for `/bin/sh`.
  Symlinks are deliberately *not* followed: on a busybox system every applet is
  a symlink to the same binary, so following them would make an allow list of
  `["cat"]` also admit `sh`. The cost is that on a usr-merged Linux `/bin/ls`
  and `/usr/bin/ls` are two spellings of one file that do not match each other —
  so write the list the way the flow names the program.
...
`{type: "exit", exitCode, lines}` at the end. The exit event fires for every
run, even one that printed nothing — which is why it is its own event rather
than a flag on the last line. It still passes through `emit`, so a path that
