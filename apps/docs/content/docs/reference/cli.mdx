...
| `--observability-addr` | address | `:39999` | Admin listen address for the probes. |

The last two configure the [observability runtime service](/runtime/observability),
which serves `/healthz` and `/readyz` on a port of its own — separate from any
`http` connector, so probes never collide with a user route and answer even for a
runtime with no HTTP source at all. Each defaults to the environment variable of
the same name (see [below](#environment-variables)).
...
The `OCTO_OBSERVABILITY*` variables are read from the **process environment only**,
