...
It is a separate flag because it is the expensive half: watching **any** block
makes the engine emit an event around **every** block in every flow, not just the
watched ones. Delivery is at-most-once, so under saturation the block counters
under-report — `octo_block_events_dropped_total` says by how much.
