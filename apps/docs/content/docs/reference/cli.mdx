...
— prints one JSON envelope instead of the bare result body:
...
| `result` | The flow's own result. Present when it was not stopped at a breakpoint. |
...
A `--mocks`-only run prints the **bare body**, like any other invoke — mocking
changes what the flow does but produces no observations, so it stays pipeable
into `jq`.
