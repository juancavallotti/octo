...
| `mode` | enum | no | `"iterate"` | `iterate` (side effects, shared message) or `map` (collect each element's body into a new array; each element runs on a clone). See [control flow](/reference/blocks/control-flow#map-mode). |
