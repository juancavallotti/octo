...
**Composite blocks** additionally use *top-level* keys — slots — that hold
...
`contextMaxTokens`, `memoryCompaction`. Each composite accepts only its own
slots: a key it does not declare fails the build (e.g.
`block "if": decode settings: json: unknown field "thne"`). A leaf block
reads only the settings it declares, so a slot written on one is ignored
rather than refused.
...
| `contextMaxTokens` | int | no | `200000` | Token budget for the whole prompt (system, tools and conversation), measured from the provider's reported usage. |
