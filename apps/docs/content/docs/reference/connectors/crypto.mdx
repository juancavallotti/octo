...
| `publicKey` | string | For `rsa-oaep` | — | PEM public key, used to encrypt. PKIX or PKCS#1, at least 2048 bits. |
| `privateKey` | string | For `rsa-oaep` | — | PEM private key, used to decrypt. PKCS#8 or PKCS#1; supplying it alone is enough, since it carries its public half. |
