...
The connector keeps a pool of idle connections so outbound requests reuse them instead of dialling — and, against HTTPS, re-handshaking — every time. The pool is the counterpart to a flow's [`workers`](/reference/flow-file#flows): `workers: 512` asks for up to 512 concurrent outbound requests, and `maxIdleConnsPerHost` decides how many of those connections survive to serve the next batch. Sizing the pool below `workers` means the excess is closed and re-dialled each round.
...
Note that `maxResponseBytes` interacts with pooling: a connection is only reusable once its response body is read to the end. The connector drains a truncated body for you, so the cap costs bandwidth rather than connections.
