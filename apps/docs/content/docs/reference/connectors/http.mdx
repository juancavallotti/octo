...
<Callout type="warn">
`EventSource` cannot send an `Authorization` header, so `jwt-validate` in front of an SSE route only works for non-browser clients. Browsers need a cookie or a query parameter.
</Callout>
