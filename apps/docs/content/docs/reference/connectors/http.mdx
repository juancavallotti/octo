...
| `methods` | string[] | No | — (every method) | HTTP methods the route answers. Naming any means every other method gets a `405` from the router, before any flow runs. A name that is not an HTTP method fails at startup. |
