...
| --- | --- | --- | --- | --- |
| `subject` | string | Yes | none | Queue subject to subscribe to. Every replica subscribed to the same subject competes for messages, so each message is handled once across the cluster. |
...
**Queue Dispatch** sends the message to a platform queue subject to load balance work across replicas. By default it publishes fire-and-forget (one-way); enable `awaitReply` to wait for one consumer's reply and fold it back in (request).

| Setting | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `subject` | expression | Yes | none | CEL expression for the queue subject to send to; enables per-message routing/sharding. |
