...
| --- | --- | --- | --- | --- |
| `subject` | string | Yes | none | Topic subject to subscribe to. Every subscriber on the subject (including every replica) receives every message: broadcast fan-out, unlike a queue's competing consumers. |
...
| Setting | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `subject` | expression | Yes | none | CEL expression for the topic subject to broadcast to; enables per-message routing. |
...
<Callout type="warn">
The runtime holds no list of system subjects and validates nothing about them. It does not know which exist, what may be published on them, or what shape a subscriber expects. An unroutable name publishes into the void, and a malformed payload is the subscriber's problem, exactly as they would be on any other subject. Check the name against the [event bus reference](/platform/event-bus) before relying on it.
