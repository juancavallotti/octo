...
The prefix buys a flow the ability to raise a platform event, not to read the platform's traffic. The unscoped plane carries `internal.logs` and `internal.traces`, which hold every deployment's records; a flow that could subscribe there would be reading workloads it has nothing to do with, wildcards included. Publishing has no equivalent reach — a publish subject cannot contain a wildcard, and the worst a wrong name does is go nowhere.
