...
cel-go's utility libraries: cleaning up strings, sorting and de-duplicating
lists, base64-encoding, rounding, turning a list into a lookup map, testing set
...
a [`foreach`](/reference/blocks/control-flow), since most collection work that
...
[RE2](https://github.com/google/re2/wiki/Syntax), with no backreferences or
...
<Callout type="info">
  ...
  backtracking, so this is a throughput consideration on large payloads rather
  than a denial-of-service surface.
</Callout>
...
| Anything user-defined | CEL has no user functions, and expressions cannot define one. A transformation too large for an expression belongs in a [`multi-transform`](/reference/blocks/data), a sub-flow, or an [AI Mapping](/ai/ai-mapping) block. |
