...
`matches` uses [RE2](https://github.com/google/re2/wiki/Syntax) syntax, with no
backreferences or lookaheads. It is also available as a global:
`matches("hello", "^h")` is the same as `"hello".matches("^h")`.

The table above is standard CEL. Octo also enables cel-go's strings library, so
`trim`, `split`, `join`, `replace`, `substring`, `upperAscii`, `lowerAscii`, and
`format` are available on top of it. See [Extension
Libraries](/reference/cel/libraries#strings).

<Callout type="warn">
  String indexing (`"abc"[0]`) is not part of CEL and fails to compile. Use
  `"abc".charAt(0)` or `"abc".substring(0, 1)`.
</Callout>
...
Sorting, de-duplicating, flattening, and slicing a list come from the lists
library, in [Extension Libraries](/reference/cel/libraries#lists).
...
`type(x)` returns a value's type and is useful in comparisons, so
`type("a") == string` evaluates to `true`. A type itself is not a
...
numbers decode as doubles, so comparisons against int literals still work:
