...
<Callout type="warn">
**This is not a network boundary.** The [observability API](/platform/observability-api-reference) performs no authentication and is reachable from anywhere on the cluster network, so a flow author can point a connector at the service's address directly. Until the API gains authentication, treat the grant as a declaration, the same footing as the orchestrator one below.
</Callout>
