...
Every deployed integration presents a platform token of its own, and what it may do is what that token carries. Every one of them reaches the stores its pod owns — its [key-value namespace](/platform/kv-and-storage), its objects, its agent memory — and the deploy and rollout dialogs carry one setting under **Advanced** saying what it reaches besides.

| Access | What the token additionally opens |
| --- | --- |
| **Serves only** (the default) | Nothing. Right for almost every integration. |
| **Builds integrations** | Integrations, their resources, versions and dev runs — across the whole installation. For an integration that builds or tests others. |
| **Runs the installation** | The above, plus deploying, rolling out, scaling and removing deployments — anyone else's included. |

A choice rather than a set of switches: the three are ordered, each the one before it plus more, so a set would only ever be spelled as its widest member.

An integration that needs more than the default, and a reason for it, is the alert-triage agent in [Agentic self-healing](/guides/agentic-self-healing).

<Callout type="warn">
This is not scoped to the deployment's own integration. "Builds integrations" reads and writes **every** integration here, and "Runs the installation" acts on **every** deployment. Grant either only to an integration you would trust with the corresponding role on your own account.
</Callout>

**Who may grant it.** The token is minted on the authority of whoever is deploying, and iam refuses to lend a deployment more than that person could do themselves. It never inherits: an administrator's deployment is not an administrator, and access is what was asked for on purpose.

**Addresses are not access.** `ORCHESTRATOR_URL` and `OBSERVABILITY_URL` are in every runtime pod — the first because the runtime needs it for the KV store and leader election, the second so a granted integration can use it without a second setting. Neither is a boundary: both APIs authorize the token they are presented, and anything on the cluster network could dial either address regardless. This replaced two checkboxes that withheld an address, which only ever hid a service from the deployments that had asked to use it.

**Renewal keeps what was granted.** A deployment's token is renewed as what it was minted as, not as what its owner may do now. Taking somebody's operator role away does not quietly change what their running integrations can reach — stopping one is what deleting the deployment is for.
