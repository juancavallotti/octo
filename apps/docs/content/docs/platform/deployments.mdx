...
**The observability grant is the address, not the reach.** The [observability API](/platform/observability-api-reference) authorizes every request by the roles on the presented token, and a deployment's own token carries `platform:runtime`, which matches no rule there. A deployment that has the address therefore reads nothing with its own credential, whatever this box says.

What does read is a request carrying **somebody else's** token: a flow reached by a person — Dr. Octo answering a question in the chat, for instance — spends the caller's token, and the read succeeds exactly when that person is allowed the read. The deployment's own token is never widened into a person's.
