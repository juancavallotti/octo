...
**The orchestrator grant is a declaration.** `ORCHESTRATOR_URL` is already in every runtime pod, because the runtime's k8s services module needs it for the [KV store](/platform/kv-and-storage) and leader election. Access control over the orchestrator API is expected to arrive as RBAC gating on this declaration, so declare it now for the integrations that genuinely call the API.
