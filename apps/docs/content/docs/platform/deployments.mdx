...
What does gate it is the token each deployment is given. The orchestrator authorizes every request by the roles on the presented token, and a deployment's own token carries `platform:runtime` — which reaches its key-value store, its objects and its agent memory, and nothing else. Declaring the grant does not widen that; a pod cannot read another deployment's data, change an integration, or see the installation's settings whatever this box says.
