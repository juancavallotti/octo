...
`POST /auth` is the exchange and `POST /auth/refresh` renews the result, re-reading roles from the database so a revocation lands within a token lifetime. ClusterIP only, and it publishes no OpenAPI: it is not a surface to expose or to hand to an agent as a tool.
