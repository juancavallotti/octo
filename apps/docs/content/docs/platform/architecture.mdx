...
- **From the public internet** (externally exposed deployments): the request hits Traefik at `https://{subdomain}.{baseDomain}`, matches the deployment's Ingress (TLS from cert-manager or a shared wildcard certificate), and is routed to the per-deployment Service and on to a runtime pod.
