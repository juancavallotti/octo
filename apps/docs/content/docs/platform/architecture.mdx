...
Authentication is Auth.js (NextAuth) with an OIDC provider and JWT sessions. There is no database adapter, so the same config backs both the route handlers and the request proxy (`proxy.ts`), which redirects unauthenticated browser navigations to sign-in and answers unauthenticated `/api/*` calls with a 401. One application endpoint bypasses the session gate because it carries its own authentication: `/mcp` (bearer tokens). The welcome page, `/api/auth*` and two static images are exempt as well. See [Users and API Keys](/platform/users-and-api-keys).
