...
| `internal.traces` | every runtime pod with tracing on | log aggregator (queue group) | [Trace](/runtime/tracing) record shipping |
...
<Callout type="info">
Both subjects are consumed by the **same service** — the one still called the log aggregator. Traces landed there because they arrive the same way and are queried the same way; the name has not caught up.
</Callout>
