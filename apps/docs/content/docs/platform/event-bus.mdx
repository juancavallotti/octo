...
The one crossing is deliberate and one-way. A flow that writes `system:` in front of a subject publishes onto the unscoped plane, which is how a deployed app raises a platform event. Nothing carries traffic the other way: platform events are not delivered into flows unless a flow subscribes to that subject by name.
