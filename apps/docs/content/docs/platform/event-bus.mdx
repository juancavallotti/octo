...
| `internal.traces` | every runtime pod with tracing on | observability service (queue group) | [Trace](/runtime/tracing) record shipping |
...
`internal.logs` and `internal.traces` are deliberately **not** scoped per deployment, unlike the `octo.{deploymentId}.*` subjects: a single service consumes each as a competing consumer across every deployment, and the deployment identity travels inside each record. Scoping them would make its subscription list grow with the number of deployments, which is the thing an aggregator exists to avoid.

<Callout type="info">
Both subjects are consumed by the **same service** — the [observability service](/platform/observability-api-reference). Traces landed there because they arrive the same way and are queried the same way.
</Callout>
