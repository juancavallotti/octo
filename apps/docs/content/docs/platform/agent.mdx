...
He is deployed with no [platform access](/platform/deployments#platform-access) of his own, which looks wrong for an agent that drives the whole API and is not. **When somebody is chatting, his tools spend that person's token**, so what he may do is what they may do — and no grant on his deployment would change that.

His own token is what an **unattended** run falls back to: an alert waking the troubleshooter, where there is nobody to borrow from and never will be. Out of the box that token opens only what his pod owns, so an unattended run can read its own memory and nothing else. Ticking the access grants on his deployment is what lets alert-driven triage read logs, traces and deployments — and it is left to you deliberately, because those grants say what a run *nobody started* may do to this installation.

<Callout>
Because he acts as whoever is asking, he hands a chat-panel user nothing they could not already reach: [Traces](/platform/traces) and [Logs](/platform/logs) are readable by any signed-in user, and a write he performs is one that person could have performed themselves. The reason to weigh is prompt injection, below.
</Callout>
