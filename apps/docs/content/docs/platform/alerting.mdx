...
For how this fits together with the pods that emit the signals and the app that consumes them, see [The self-healing loop](/platform/self-healing-loop). For the whole thing built end to end, from this action to an agent that triages the alert and asks a person in Slack before it changes anything, see [Agentic self-healing](/guides/agentic-self-healing).
...
![The Alerts list: four watches with their severity, phase, last value, schedule and last run, one firing, one pending and one muted](/screenshots/platform-watch-table.png)
...
![The execution log with the filter off: six evaluations newest first, one expanded to show the condition it judged, the value observed and the threshold it was judged against](/screenshots/platform-evaluation-history.png)
