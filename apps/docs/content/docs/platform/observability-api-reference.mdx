...
Every route needs a bearer token minted by this installation's [iam service](/platform/users-and-api-keys), except `/healthz` and the two description endpoints. The service fetches iam's published keys through discovery and verifies the signature, the issuer, the audience and both time claims itself.

What a route requires depends on the [roles](/platform/users-and-api-keys#roles) the token carries:
