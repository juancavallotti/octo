...
<Callout type="info">
Volatile namespaces fall back to Postgres whenever no Redis backs them — which today is always (the routing is not wired up yet), and afterwards whenever the platform has none configured (`redis.enabled: false` and no `externalRedis`). Nothing breaks either way; you simply pay persistent-tier cost for volatile data. Write against the guarantee rather than the backend: it is the guarantee that will not change.
</Callout>
