...
<Callout type="info">
When the platform has no Redis configured (`redis.enabled: false` and no `externalRedis`), volatile namespaces quietly fall back to Postgres. Nothing breaks; you simply pay persistent-tier cost for volatile data.
</Callout>
