...
On the platform, an integration is a named project stored in Postgres: a display name plus the files it is made of — its flow files and the [resources](/platform/resources-and-secrets) beside them. This page covers the integration record itself: its lifecycle, naming, attribution, and folder organization. Versioning is covered in [Snapshots](/platform/snapshots) and running in [Deployments](/platform/deployments).

![The integrations list organized into folders](/screenshots/platform-integrations.png)

## The integration record

Each integration row carries four things.
...
`definition` is the raw integration YAML, exactly as the editor (or an MCP client) saved it. It is not a stored field: it is the integration's flow files merged, on the same terms the runtime merges a directory — flow, connector and processor names must be unique across them, and `service:` may be declared in only one. An integration with a single flow file, which is how every integration starts, gets that file back byte for byte.

Files are stored one row each, and what a file *is* follows from its path, exactly as it does when `--config` points at a folder:

| Path | Role |
| --- | --- |
| a root-level `*.yaml` / `*.yml` | a flow file, merged into the definition |
| a root-level `*_test.yaml` | a [dolphin](/testing/dolphin) suite |
| anything else — nested paths, dotfiles | a [resource](/platform/resources-and-secrets) |
...
- The MCP endpoint (`/mcp`), where AI agents create and update integrations programmatically. A write publishes a live-reload event so an editor with that integration open refreshes. See [The Platform MCP Server](/ai/platform-mcp).
- The orchestrator API, for any authenticated automation the BFF fronts.
