import { runningPort } from "../../../../api/run/session";
import { isValidNamespace } from "../../../../api/run/namespace";
...
async function proxy(req: Request, { params }: Params): Promise<Response> {
  ...
  if (port === null) {
    return new Response("no running networked integration for this run", { status: 404 });
  }
  ...
  try {
    ...
    return new Response(`run proxy error: ${(err as Error).message}`, { status: 502 });
  }
