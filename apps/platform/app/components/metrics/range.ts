 ...
 * tier, and every window that is neither has to be resolved to one of them by
 * the service — silently and lossily, so asking for five minutes of a two-minute
 ...
 * bucket is are both per-install settings. Live and Historic are true whatever
 * the numbers are, and the header states what those numbers turned out to be.
 */
...
export interface ViewPreset {
  ...
  /**
   * How far back to ask before the pods have said how far back there is. Only a
   * first guess: once /pods has answered, `reachFor` replaces it with the tier's
   * own configured reach.
   */
  askMs: number;
 ...
 * link should show the default view, not an error page.
 */
 ...
 * is wide enough to show five seconds of change. The overview is what a reader
 ...
 *
 * Deduced rather than assumed: retention, the bucket width, and the live tier's
 * depth that follows the bucket are all per-install settings, so a hard-coded
 ...
 *
 * Falls back to the view's own guess when no pod has reported.
 */
