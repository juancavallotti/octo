...
const WHAT_IT_DOES: Record<string, string> = {
  postgres:
    "Integrations, deployments, stored logs and traces. Nothing works without it.",
  redis:
    "Shared state between replicas — today, the fold that keeps streamed traces small.",
  nats: "Live updates: deployment status, and the telemetry runtimes ship to the aggregator.",
  kubernetes:
    "Deploying anything. Without it the platform is an editor and a database.",
  embeddings:
    "Ranking a search of agent memory by meaning. Without it, search matches text.",
};
    ...
    <div className="border-b border-black/[0.06] py-3 last:border-b-0 dark:border-white/[0.08]">
      <div className="flex items-baseline justify-between gap-4">
        <span className="text-sm font-medium">
          {LABELS[dep.name] ?? dep.name}
        </span>
        <Badge dep={dep} />
      </div>
      <p className="mt-1 text-xs text-zinc-500">
        {WHAT_IT_DOES[dep.name] ?? ""}
      </p>
      ...
      {dep.detail && (
        <p className="mt-1 font-mono text-xs break-words text-red-500">
          {dep.detail}
        </p>
      )}
    </div>
    ...
    <div className="flex h-full flex-col overflow-y-auto px-6 py-5">
      <div className="mx-auto w-full max-w-2xl">
        ...
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          What this installation runs on, and whether the orchestrator can reach
          it right now. Each check is one round trip — enough to say a
          dependency answered, not enough to say it is healthy.
        </p>
