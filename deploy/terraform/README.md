...
| `modules/octo-secrets` | every root — the credentials, installed as Secrets before the chart |
...
### Credentials never reach Helm

Every credential an octo installation needs — the bundled Postgres password, the
Auth.js session secret, the OIDC client secret, the KV encryption key, the
dev-run HMAC key, the embedding provider key — is created as a **Kubernetes
Secret** by `modules/octo-secrets` before the chart is applied, and reaches the
chart as the *name* of that Secret.

That is not a stylistic preference. Helm keeps every value it is given in the
release history: `set_sensitive` marks a value secret to Terraform's plan output,
not to the release Secret that Helm writes into the cluster. A credential passed
as a chart value is therefore readable, by anyone who can read Secrets in the
namespace, for as long as the revision carrying it is retained — which is long
after the rotation that was supposed to end it. `modules/helm-release` has no
`set_sensitive` block at all, and adding one undoes this.

The credentials are still generated by the roots and still live in their state,
which this setup says plainly: there is no Secret Manager here. State is one
place, versioned and encrypted at rest in its bucket, and a rotation replaces it.
Release history is every retained revision, in the cluster, and forgets nothing.
