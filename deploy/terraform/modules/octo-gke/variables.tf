# Inputs to the shared GKE composition. The two roots (gke-standard, gke-autopilot)
# surface these same names in their own variables.tf with flavour-appropriate
# defaults, so a user edits terraform.tfvars and never reads this file.

variable "autopilot" {
  type        = bool
  description = "Run GKE Autopilot rather than Standard. Autopilot manages the nodes, bills every pod on its resource requests, rejects hostPath/hostPort outright, and can take noticeably longer to start a pod because it may provision a node first. It also selects which helm/values-gke-*.yaml profile is installed."
}
...
#
# The two secrets the chart also needs — the Auth.js session secret and the KV
# encryption key — are deliberately NOT inputs. They are minted per cluster and
# held in state, because neither has anything to stay consistent with across a
...
# The node-pool inputs below are ignored entirely when autopilot = true: Google
# owns the nodes there, and the cluster module creates no node pool at all.

variable "machine_type" {
  type        = string
  description = "Node machine type. The GKE Standard profile requests roughly 2 vCPU / 6Gi across its workloads before ingress-nginx and cert-manager, so two e2-standard-2 is the comfortable floor. Ignored on Autopilot."
...
variable "spot_nodes" {
  type        = bool
  description = "Use Spot VMs: far cheaper, preemptible at any moment."
  default     = true
}

variable "regional" {
  type        = bool
  description = "Spread the control plane and one node pool per zone across the region. Off by default — a zonal cluster is cheaper and quicker to create, and node_count is PER ZONE, so turning this on triples the nodes."
  default     = false
}
