...
module "secrets" {
  ...
  # Plan-known booleans, not "is the credential non-empty": these credentials are
  # generated by random_password/random_bytes and unknown until apply, and a count
  # that depends on an unknown fails the plan before anything is created.
...
module "octo" {
  ...
  depends_on = [module.addons, module.secrets]
}
