...
module "secrets" {
  ...
  # Which Secrets to create, as plan-known booleans. They cannot be derived from
  # the credentials themselves: these are generated by random_password/random_bytes
  # and unknown until apply, and a count that depends on an unknown fails the plan
  # outright. This root always runs the bundled Postgres and always wants both keys.
...
module "octo" {
  ...
  # The Secrets and the namespace must exist before the chart that reads them.
  # Explicit because modules/octo-secrets' outputs are plan-known constants and so
  # carry no implicit dependency of their own — see its outputs.tf.
  depends_on = [null_resource.pull_images, module.secrets]
}
