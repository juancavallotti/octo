...
func gatedAgentConfig(authorize string, events *types.FlowConfig) types.BlockConfig {
	...
	return types.BlockConfig{
		...
		Tools:          []types.ToolConfig{tool},
		MemoryThreadID: "body.threadId",
		Input:          `has(body.message) ? body.message : ""`,
		AuthorizeID:    `has(body.authorizationId) ? body.authorizationId : ""`,
		AuthorizeAllow: `has(body.allow) && body.allow == true`,
		Emit:           []string{eventToolAuthorization, eventToolCall, eventToolResult, eventSignal},
		Events:         events,
		MaxIterations:  4,
	}
}
...
func TestTheGateReadsTheArguments(t *testing.T) {
	...
	path := eventsPath(types.Settings{})
	cfg := gatedAgentConfig(`input.method != "GET"`, &path)
	cfg.AuthorizeTimeout = "10m" // never reached: nothing should wait
	block := mustBuildAI(t, reg, depsLLM(fake), cfg)
...
func TestAnUnanswerableGateIsRefusedAtBuild(t *testing.T) {
	...
	cases := []struct {
		...
		{"no way to be told", "authorizeId is required", func(cfg *types.BlockConfig) {
			cfg.AuthorizeID, cfg.AuthorizeAllow = "", ""
		}},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.edit(&cfg)
			_, err := (&builder{
				reg: answeringRegistry(&seen, &events, nil), pool: pool.New(0, 0),
...
func TestAnMCPRouterRefusesAToolGate(t *testing.T) {
	...
	tool := toolBranch("write", "changes something", types.Settings{})
	tool.Authorize = "true"
	_, err := (&builder{reg: recordRegistry(&seen), pool: pool.New(0, 0)}).block(types.BlockConfig{
		Type: "mcp-router", Name: "router", Tools: []types.ToolConfig{tool},
	})
