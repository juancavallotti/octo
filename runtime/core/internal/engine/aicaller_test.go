...
// mappingRegistry is the shared AI test registry plus the ai-mapping leaf, which
// registers itself into the global block registry rather than the test one.
func mappingRegistry(seen *[]any) *core.BlockRegistry {
	reg := recordRegistry(seen)
	reg.MustRegister(blockTypeAIMapping, newAIMapping)
	return reg
}
...
func TestMappingTracesItsOneCall(t *testing.T) {
	...
	if got := turns[0].Path; got != ".reshape" {
...
func embedRegistry(seen *[]any) *core.BlockRegistry {
	reg := recordRegistry(seen)
	reg.MustRegister(blockTypeAIEmbed, newAIEmbed)
	return reg
}
