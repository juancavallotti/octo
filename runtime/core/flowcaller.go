...
// FlowCaller invokes a registered flow by name. A flow without an external source
// registers its input channel under its name, and a caller pushes messages into
// it, correlating the result through the flow-event bus.
//
