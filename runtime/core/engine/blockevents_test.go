...
func TestBlockEventsNestIntoComposites(t *testing.T) {
	...
	rec := runWithEvents(t, testRegistry(), types.FlowConfig{
		Name: "orders",
		Process: []types.BlockConfig{
			{Type: "enrich", Name: "lookup", Settings: types.Settings{"body": &body}},
	...
	tests := []struct {
	...
	}{
		{
			...
			block: types.BlockConfig{
				...
				Settings: types.Settings{"process": []types.BlockConfig{leaf("work")}, "error": []types.BlockConfig{leaf("recover")}}},
		...
		},
		{
			...
			block: types.BlockConfig{
				Type: "if", Name: "check",
				Settings: types.Settings{"condition": "true", "then": chain("yes"), "else": chain("no")}},
		...
		},
		{
			...
			block: types.BlockConfig{
				Type: "if", Name: "check",
				Settings: types.Settings{"condition": "false", "then": chain("yes"), "else": chain("no")}},
		...
		},
		{
			...
			block: types.BlockConfig{
				Type: "switch", Name: "route",
				Settings: types.Settings{"cases": []map[string]any{{"when": "true", "process": []types.BlockConfig{leaf("anon")}}}}},
		...
		},
		{
			...
			block: types.BlockConfig{
				Type: "switch", Name: "route",
				Settings: types.Settings{"cases": []map[string]any{{"when": "false", "process": chain("never").Process}}, "default": chain("fallback")}},
		...
		},
		{
			...
			block: types.BlockConfig{
				...
				Settings: types.Settings{"branches": []types.FlowConfig{{Name: "audit", Process: []types.BlockConfig{leaf("record")}}}}},
		...
		},
		{
			...
			block: types.BlockConfig{
				Type: "fork", Name: "fanout",
				Settings: types.Settings{"branches": []types.FlowConfig{{Process: []types.BlockConfig{leaf("record")}}}}},
			want: "orders.fanout[0].record",
		},
		{
			name:  "enrich body",
			block: types.BlockConfig{Type: "enrich", Name: "lookup", Settings: types.Settings{"body": chain("fetch")}},
			want:  "orders.lookup[body].fetch",
		},
		{
			name: "foreach body",
			block: types.BlockConfig{
				Type: "foreach", Name: "each",
				Settings: types.Settings{"items": "[1]", "as": "item", "body": chain("handle")}},
...
func TestBlockEventsOnlyReachTheWiredDispatcher(t *testing.T) {
	cfg := types.FlowConfig{Name: "orders", Process: []types.BlockConfig{
		{Type: "enrich", Name: "lookup",
			Settings: types.Settings{"body": &types.FlowConfig{
