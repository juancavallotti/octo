...
func TestStopShortCircuitsChain(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: pool.New(0, 0)}).flow(types.FlowConfig{
...
func TestStopBubblesThroughComposite(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: pool.New(0, 0)}).flow(types.FlowConfig{
		Process: []types.BlockConfig{
			{
				Type:     "if",
				Settings: types.Settings{"condition": "true", "then": &types.FlowConfig{Process: []types.BlockConfig{{Type: "stop"}}}}},
			{Type: "record"},
		},
	})
	if err != nil {
...
func TestStopBubblesThroughFork(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: p}).flow(types.FlowConfig{
		Process: []types.BlockConfig{
			{
				Type: "fork",
...
func TestStopFromEveryForkBranch(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: p}).flow(types.FlowConfig{
		Process: []types.BlockConfig{{Type: "fork", Settings: types.Settings{"branches": branches}}, {Type: "record"}},
	})
	if err != nil {
		t.Fatalf("build flow: %v", err)
	}
...
func TestStopBubblesThroughEnrich(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: pool.New(0, 0)}).flow(types.FlowConfig{
		Process: []types.BlockConfig{
			{
				Type:     "enrich",
				Settings: types.Settings{"body": &types.FlowConfig{Process: []types.BlockConfig{{Type: "stop"}}}, "setVars": map[string]string{"seen": "'yes'"}}},
			{Type: "record"},
		},
	})
	if err != nil {
...
func TestStopHaltsForeach(t *testing.T) {
	...
	flow, err := (&builder{reg: reg, sched: pool.New(0, 0)}).flow(types.FlowConfig{
		Process: []types.BlockConfig{{
			Type:     "foreach",
			Settings: types.Settings{"items": "[1, 2, 3, 4]", "body": &types.FlowConfig{Process: []types.BlockConfig{{Type: "count-then-stop"}}}}}},
	})
	if err != nil {
		t.Fatalf("build flow: %v", err)
	}
