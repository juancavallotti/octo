		...
		Process: []types.BlockConfig{
			{
				Type: "if",
				Name: "checkHeader",
				Settings: types.Settings{"condition": "false", "then": &types.FlowConfig{Process: []types.BlockConfig{
					wrap("target", types.BlockConfig{Type: "mark", Settings: types.Settings{"value": "target"}}),
				}}}},
			{Type: "record"},
		},
...
func TestBreakpointPreservesErrorLabel(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			target: types.BlockConfig{
				...
				Settings: types.Settings{"condition": "true", "then": &types.FlowConfig{Process: []types.BlockConfig{{Type: "boom", Name: "inner"}}}}},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			plain, err := (&builder{reg: reg, sched: pool.New(0, 0)}).flow(types.FlowConfig{
			...
			var blockErr *core.BlockError
			if !errors.As(brokenErr, &blockErr) {
				t.Fatal("wrapped error is not a *core.BlockError")
			}
...
func TestBreakpointCannotBeAuthored(t *testing.T) {
	_, err := (&builder{reg: breakpointRegistry(new(bool)), sched: pool.New(0, 0)}).flow(types.FlowConfig{
		Process: []types.BlockConfig{
			{Type: blockKindBreakpoint, Settings: types.Settings{"process": []types.BlockConfig{{Type: "mark"}}}},
		},
	})
	if err == nil {
