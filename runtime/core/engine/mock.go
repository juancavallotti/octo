...
func (b *mockBlock) Process(_ context.Context, msg *types.Message) (*types.Message, error) {
	for i := range b.cases {
		matched, err := expr.EvalCondition(b.cases[i].when, msg, b.env)
		if err != nil {
...
func (b *builder) mock(raw types.Settings) (core.MessageProcessor, error) {
	if b.deps.Mocks == nil {
		return nil, errMockUnwired
	}
	// A mock replaces a block, so it holds no sub-flows — not even the ones the block
	// it replaced had — and nothing but the address the injector wrote.
	var settings mockSettings
	if err := raw.DecodeStrict(&settings); err != nil {
		return nil, err
	}
