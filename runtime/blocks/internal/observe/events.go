...
// New builds an emitter over the events sub-flow a block declared. kinds is the
// validated allow-list from Kinds, or nil to emit everything; label is the block
// type the log lines name.
func New(flow core.MessageProcessor, kinds map[string]bool, label string) *Emitter {
	return &Emitter{flow: flow, kinds: kinds, label: label}
}
...
// Kinds validates an emit list against the block's own vocabulary, returning
...
func Kinds(label string, known map[string]bool, emit []string) (map[string]bool, error) {
	if len(emit) == 0 {
		return nil, nil
	}
