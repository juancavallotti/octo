...
func steerableAgentConfig() types.BlockConfig {
	return types.BlockConfig{
		...
		Settings: types.Settings{"connector": "claude", "prompt": "work", "tools": []map[string]any{toolBranch("lookup", "looks things up", nil)}, "memoryThreadId": "body.threadId", "input": "body.message", "maxIterations": 6}}
}
...
func TestStopWhenEndsTheRunInFlight(t *testing.T) {
	...
	cfg.Settings["stopWhen"] = "has(body.stop)"
...
func TestAMessageAcceptedAtTheIterationCapIsReported(t *testing.T) {
	...
	path := eventsPath(nil)
	cfg.Settings["events"] = &path
	cfg.Settings["emit"] = []string{eventSignal}
	// The cap has to be reachable, so the run needs somewhere to land.
	cfg.Settings["default"] = &types.FlowConfig{
	...
	reg := eventsRecordingRegistry(&seen, &events, func() {
		...
		if calls != cfg.Settings["maxIterations"] {
			return
		}
