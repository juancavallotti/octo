...
func compactingAgentConfig(budget int) types.BlockConfig {
	return types.BlockConfig{
		...
		Settings: types.Settings{"connector": "claude", "prompt": "work", "tools": []map[string]any{toolBranch("lookup", "looks things up", nil)}, "contextMaxTokens": budget, "maxIterations": 4}}
}
