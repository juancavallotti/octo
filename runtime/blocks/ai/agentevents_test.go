...
func agentWithEvents(events *types.FlowConfig, emit []string, stream bool) types.BlockConfig {
	return types.BlockConfig{
		Type:     "ai-agent",
		Settings: types.Settings{"connector": "claude", "prompt": "do the task", "tools": []map[string]any{toolBranch("look", "look something up", types.Settings{})}, "events": events, "emit": emit, "stream": stream}}
}
...
func TestAIAgentEventsBuildValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := tryBuildBlock(reg, depsLLM(tc.conn), tc.cfg)
			if err == nil {
	...
	if _, err := tryBuildBlock(reg, depsLLM(&streamingLLM{}), agentWithEvents(&path, nil, true)); err != nil {
