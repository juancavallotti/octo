...
func decodeMCPRouter(raw types.Settings) (mcpRouterSettings, error) {
	var cfg mcpRouterSettings
	if err := raw.DecodeStrict(&cfg); err != nil {
		return cfg, err
	}
	if len(cfg.Tools) == 0 && len(cfg.Resources) == 0 && len(cfg.Prompts) == 0 {
		return cfg, errors.New("mcp-router block requires at least one tool, resource, or prompt")
	}
	return cfg, nil
}

// mcpServerName is what the router reports in its initialize response: the
// configured name, else the block's, else the default.
func mcpServerName(cfg mcpRouterSettings, deps core.BlockDeps) string {
	if cfg.ServerName != "" {
		return cfg.ServerName
	}
	if deps.Address.Name != "" {
		return deps.Address.Name
	}
	return defaultMCPServerName
}

//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newMCPRouter(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	cfg, err := decodeMCPRouter(raw)
	if err != nil {
		return nil, err
	}
	...
	specs, err := mcpToolSpecs(cfg.Tools)
	if err != nil {
		return nil, err
	}
	branches, tools, err := b.buildTools(blockKindMCPRouter, specs)
	if err != nil {
		return nil, err
	}
	...
	return &mcpRouter{
		serverName: mcpServerName(cfg, deps),
		tools:      tools,
		branches:   branches,
		toolMeta:   toolMeta,
		resources:  resources,
...
// mcpToolSpecs reduces the router's tool settings to what building a branch
// needs; the MCP-only metadata is collected separately by buildMCPToolMeta.
//
// An authorize condition is refused: a gate needs somewhere to ask and a run to
// hold the call while it waits, and an mcp-router has neither — a tool call
// arrives over the protocol, is answered, and is gone.
func mcpToolSpecs(tools []mcpToolSettings) ([]toolSpec, error) {
	specs := make([]toolSpec, len(tools))
	for i, t := range tools {
		if t.Authorize != "" {
			return nil, fmt.Errorf(
				"mcp-router tool %q: authorize is an ai-agent setting and has no effect here; "+
					"an MCP client runs its own consent step", t.Name)
		}
...
func buildMCPResources(configs []mcpResourceSettings) (fixed, templated []mcpResource, err error) {
...
func buildMCPResource(i int, c mcpResourceSettings) (mcpResource, string, error) {
	switch {
...
func buildMCPPrompts(configs []mcpPromptSettings) ([]mcpPrompt, error) {
...
func buildMCPToolMeta(configs []mcpToolSettings) (map[string]mcpToolMeta, error) {
...
func toolAnnotations(a *mcpToolAnnotations) map[string]any {
	if a == nil {
		return nil
	}
