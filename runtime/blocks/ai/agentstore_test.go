...
func buildStoreAgent(t *testing.T, cfg types.BlockConfig) (core.MessageProcessor, error) {
	...
	proc, err := tryBuildBlock(agentRegistry(&seen), depsLLM(conn), cfg)
	if err != nil {
		return nil, err
	}
	return proc, nil
...
func TestAgentStoreRecordsCompletedTurn(t *testing.T) {
	...
	cfg.Settings["input"] = `"how do I get a refund?"`
...
func TestAgentStoreReportsTheTitleItWrote(t *testing.T) {
	...
	cfg := storeAgentConfig("support")
	cfg.Settings["input"] = `"how do I get a refund?"`
	cfg.Settings["events"] = &types.FlowConfig{Process: []types.BlockConfig{{Type: "collect"}}}
...
func TestAgentStoreNamesWithTheChain(t *testing.T) {
	cfg := storeAgentConfig("support")
	cfg.Settings["input"] = `"how do I get a refund?"`
	cfg.Settings["nameThread"] = &types.FlowConfig{Process: []types.BlockConfig{{
		Type:     "tool",
...
func TestAgentStoreHistoryOff(t *testing.T) {
	cfg := storeAgentConfig("support")
	cfg.Settings["history"] = historyOff
...
func TestAgentStoreVolatileCanStillOptIntoHistory(t *testing.T) {
	cfg := storeAgentConfig("specialist")
	cfg.Settings["memoryVolatile"] = true
	cfg.Settings["history"] = historyRecord
	mem, _, _ := runStoreAgent(t, cfg, endTurnResp("done"))
...
func TestAgentStoreRecordsUnansweredQuestion(t *testing.T) {
	cfg := storeAgentConfig("support")
	cfg.Settings["input"] = `"will this ever finish?"`
	cfg.Settings["maxIterations"] = 1
	cfg.Settings["default"] = &types.FlowConfig{Process: []types.BlockConfig{{Type: "record", Name: "guard"}}}
...
func TestAgentStoreConfigRequiresAgentID(t *testing.T) {
	cases := []struct {
		...
		{"history without agentId", "agentId", func(c *types.BlockConfig) { c.Settings["history"] = historyRecord }},
		{"userMemory without agentId", "agentId", func(c *types.BlockConfig) { c.Settings["userMemory"] = true }},
		{"agentId without a thread", "memoryThreadId", func(c *types.BlockConfig) {
			c.Settings["agentId"] = "support"
			c.Settings["memoryThreadId"] = ""
		}},
		{"userMemory without a userId", "userId", func(c *types.BlockConfig) {
			c.Settings["agentId"] = "support"
			c.Settings["userMemory"] = true
		}},
		{"unknown history mode", "history", func(c *types.BlockConfig) {
			c.Settings["agentId"] = "support"
			c.Settings["history"] = "sometimes"
		}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cfg := memoryAgentConfig(`"thread-1"`)
			tc.cfg(&cfg)
			err := validateAgentConfig(agentSettingsOf(t, cfg))
			if err == nil {
...
func TestAgentUserMemoryRoundTrip(t *testing.T) {
	cfg := storeAgentConfig("support")
	cfg.Settings["userId"] = `"alice"`
	cfg.Settings["userMemory"] = true
...
func TestAgentMemoryToolNameCollision(t *testing.T) {
	...
	cfg.Settings["userId"] = `"alice"`
	cfg.Settings["userMemory"] = true
	cfg.Settings["tools"] = []map[string]any{toolBranch(rememberToolName, "shadows the built-in", nil)}
...
func TestAgentMemoryToolNamesAreNotClaimedWhenOff(t *testing.T) {
	cfg := storeAgentConfig("support")
	cfg.Settings["tools"] = []map[string]any{toolBranch(rememberToolName, "the flow's own tool", nil)}
...
func TestAgentStoreRejectsWhitespaceAgentID(t *testing.T) {
	...
	cfg.Settings["agentId"] = "   "
	cfg.Settings["history"] = historyRecord
	if err := validateAgentConfig(agentSettingsOf(t, cfg)); err == nil {
...
func TestNewAgentBlockBuildsWithoutAnAgentID(t *testing.T) {
	field, ok := reflect.TypeFor[agentSettings]().FieldByName("History")
	if !ok {
		t.Fatal("agentSettings has no History field")
	}
	...
	if err := validateAgentConfig(agentSettingsOf(t, memoryAgentConfig(`"thread-1"`))); err != nil {
