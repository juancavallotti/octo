...
// builder is the build-time context an AI block's configure steps share: the
// deps it was handed and the sub-flow seam positioned at the block.
type builder struct {
	deps  core.BlockDeps
	flows core.SubFlowBuilder
}

// newBuilder resolves the sub-flow seam once, so every step that builds a chain
// fails the same way on a block built outside the engine.
func newBuilder(deps core.BlockDeps) (*builder, error) {
	flows, err := core.SubFlowsOf(deps)
	if err != nil {
		return nil, err
	}
	return &builder{deps: deps, flows: flows}, nil
}
...
type aiRouter struct {
	...
	system    string
	tools     []core.LLMTool
	routes    map[string]core.MessageProcessor
...
// decodeRouter reads and checks an ai-router's settings.
func decodeRouter(raw types.Settings) (routerSettings, error) {
	var cfg routerSettings
	if err := raw.DecodeStrict(&cfg); err != nil {
		return cfg, err
	}
	if len(cfg.Routes) == 0 {
		return cfg, errors.New("ai-router block requires at least one route")
	}
	if strings.TrimSpace(cfg.Prompt) == "" {
		return cfg, errors.New("ai-router block requires a prompt")
	}
	return cfg, nil
}

//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newAIRouter(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	cfg, err := decodeRouter(raw)
	if err != nil {
		return nil, err
	}
	b, err := newBuilder(deps)
	if err != nil {
		return nil, err
	}

	caller, err := resolveLLM(blockKindAIRouter, cfg.Connector, deps)
	if err != nil {
		return nil, err
	}

	routes := make(map[string]core.MessageProcessor, len(cfg.Routes))
	...
	for i := range cfg.Routes {
		...
		flow, flowErr := b.flows.Member(route.Name, i, types.FlowConfig{Process: route.Process})
		if flowErr != nil {
	...
	if cfg.Default != nil {
		guardrail, defErr := b.flows.Branch(core.BranchDefault, *cfg.Default)
		if defErr != nil {
...
//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newAIRetry(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	var cfg retrySettings
	if err := raw.DecodeStrict(&cfg); err != nil {
		return nil, err
	}
	...
	caller, err := resolveLLM(blockKindAIRetry, cfg.Connector, deps)
	if err != nil {
		return nil, err
	}
	...
	main, err := b.flows.Branch(core.BranchProcess, types.FlowConfig{Process: cfg.Process})
	if err != nil {
	...
	if len(cfg.Error) > 0 {
		alternative, altErr := b.flows.Branch(core.BranchError, types.FlowConfig{Process: cfg.Error})
		if altErr != nil {
...
func (r *aiRetry) Process(ctx context.Context, msg *types.Message) (*types.Message, error) {
	...
	core.SetErrorVariable(msg, r.name, err)
...
func (r *aiRetry) stateText(msg *types.Message) string {
	body, _ := msg.BodyJSON()
	errInfo, _ := json.Marshal(msg.Variables[core.ErrorVariable])
...
type aiAgent struct {
	...
	// stated by the author and never derived; see agentSettings.AgentID.
	agentID string
...
func validateAgentConfig(cfg agentSettings) error {
	if len(cfg.Tools) == 0 {
	...
	// Named rather than left to the strict decoder, because "unknown field" is not
	// the useful half of the sentence: the budget now covers the system prompt and
	// the tool schemas as well as the transcript, which is why it was renamed.
	...
	return validateAgentMemoryConfig(cfg)
}

//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newAIAgent(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	...
	caller, err := resolveLLM(blockKindAIAgent, cfg.Connector, deps)
	if err != nil {
		return nil, err
	}

	specs, err := agentToolSpecs(cfg.Tools)
	if err != nil {
		return nil, err
	}
	branches, tools, err := b.buildTools(blockKindAIAgent, specs)
	if err != nil {
		return nil, err
	}
	skills, tools, err := agentToolSet(cfg, tools)
	if err != nil {
		return nil, err
	}
...
// agentToolSet completes the model-facing tool list with the implicit tools the
// block's settings ask for: load_skill when skills are declared, and the memory
// tools when user memory is on.
func agentToolSet(cfg agentSettings, tools []core.LLMTool) ([]agentSkill, []core.LLMTool, error) {
	skills, err := agentSkills(cfg.Skills, tools)
	if err != nil {
		return nil, nil, err
	}
	if len(skills) > 0 {
	...
	return skills, tools, nil
}

// agentToolSpecs reduces the agent's tool settings to what building a branch
// needs, refusing the MCP-only metadata on the way.
//
...
func agentToolSpecs(tools []agentToolSettings) ([]toolSpec, error) {
	specs := make([]toolSpec, len(tools))
	for i, t := range tools {
		switch {
		case t.Title != "":
			return nil, fmt.Errorf("ai-agent tool %q: title is mcp-router metadata and has no effect here", t.Name)
		case t.Annotations != nil:
			return nil, fmt.Errorf(
				"ai-agent tool %q: annotations are mcp-router metadata and have no effect here", t.Name)
		case t.OutputSchema != "":
			return nil, fmt.Errorf(
				"ai-agent tool %q: outputSchema is mcp-router metadata and has no effect here", t.Name)
		}
		specs[i] = toolSpec{name: t.Name, description: t.Description, inputSchema: t.InputSchema, process: t.Process}
	}
	return specs, nil
}
...
func (b *builder) configureAgentGuardrail(block *aiAgent, cfg agentSettings) error {
	if cfg.Default == nil {
		return nil
	}
	guardrail, err := b.flows.Branch(core.BranchDefault, *cfg.Default)
	if err != nil {
...
func (b *builder) configureAgentInput(block *aiAgent, cfg agentSettings) error {
...
func (b *builder) configureAgentMemory(block *aiAgent, cfg agentSettings) error {
...
func (b *builder) configureAgentNamer(block *aiAgent, cfg agentSettings) error {
	...
	flow, err := b.flows.Branch(core.BranchNameThread, *cfg.NameThread)
	if err != nil {
...
func (b *builder) configureAgentEvents(block *aiAgent, cfg agentSettings) error {
	if cfg.Events == nil {
		switch {
		case cfg.Stream:
	...
	flow, err := b.flows.Branch(core.BranchEvents, *cfg.Events)
	if err != nil {
		return fmt.Errorf("ai-agent events: %w", err)
	}
	kinds, err := observe.Kinds(blockKindAIAgent, agentEventKinds, cfg.Emit)
	if err != nil {
		return err
	}
	block.events = observe.New(flow, kinds, blockKindAIAgent)
...
// buildTools builds the tool branches and their model-facing definitions,
...
func (b *builder) buildTools(
	kind string, specs []toolSpec,
) (map[string]core.MessageProcessor, []core.LLMTool, error) {
	branches := make(map[string]core.MessageProcessor, len(specs))
	tools := make([]core.LLMTool, 0, len(specs))
	for i := range specs {
		tool := specs[i]
		if tool.name == "" {
		...
		schema, err := toolInputSchema(kind, tool)
		if err != nil {
			return nil, nil, err
		}
		// A tool branch is addressed by its own name, which is required above.
		flow, err := b.flows.Member(tool.name, i, types.FlowConfig{Process: tool.process})
		if err != nil {
			return nil, nil, fmt.Errorf("%s tool %q: %w", kind, tool.name, err)
		}
...
func agentSkills(configs []skillSettings, tools []core.LLMTool) ([]agentSkill, error) {
	if len(configs) == 0 {
		return nil, nil
	}
...
func dispatchToolBranch(
	ctx context.Context, flow core.MessageProcessor, args json.RawMessage, current *types.Message,
...
func toolInputSchema(kind string, tool toolSpec) (json.RawMessage, error) {
	schema := strings.TrimSpace(tool.inputSchema)
	if schema == "" {
		return json.RawMessage(`{"type":"object"}`), nil
	}
	if !json.Valid([]byte(schema)) {
		return nil, fmt.Errorf("%s tool %q: inputSchema is not valid JSON", kind, tool.name)
	}
...
func buildRouterSystem(prompt string, routes []routeSettings, guardrail string) string {
	var b strings.Builder
