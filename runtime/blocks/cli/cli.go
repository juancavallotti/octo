...
// blockKindCLIRun is the block type this package registers.
const blockKindCLIRun = "cli-run"

// init is this module's manifest: it registers the cli-run block and its editor
// schema.
func init() {
	core.MustRegisterBlock(blockKindCLIRun, newCLIRun)
	core.RegisterBlockMeta(core.BlockMeta{
		Type:     blockKindCLIRun,
		Label:    "Run Command",
		...
		Config: reflect.TypeFor[cliRunSettings](),
	})
}

// cliRunSettings configures the cli-run block.
type cliRunSettings struct {
...
func newCLIRun(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	var cfg cliRunSettings
	...
	block := &cliRun{command: cmd, env: expr.EnvActivation(deps.Env), onExit: onExit, name: name}
	if block.program, err = compileCLI(deps, cfg.Program, "program"); err != nil {
		return nil, err
	}
	if block.allowed, err = buildAllowList(cfg, name, block.program, block.env); err != nil {
		return nil, err
	}
	if block.args, err = compileCLI(deps, cfg.Args, "args"); err != nil {
		return nil, err
	}
	if block.stdin, err = compileCLI(deps, cfg.Stdin, "stdin"); err != nil {
		return nil, err
	}
	if err := configureCLIEvents(block, deps, cfg); err != nil {
		return nil, err
	}
	return block, nil
}
...
func buildCommand(cfg cliRunSettings) (*command, error) {
	if cfg.Program == "" {
	...
	if len(cfg.Allow) == 0 {
		...
		if !isConstant {
			slog.Warn("cli-run has no allow list and takes its program from the message, so it "+
				...
				"block", blockLabel(name), "program", cfg.Program)
			return nil, nil
		}
...
func compileCLI(deps core.BlockDeps, expression, field string) (*expr.Program, error) {
	if expression == "" {
		return nil, nil //nolint:nilnil // an unset optional expression is not an error
	}
	prog, err := expr.CompileMessage(deps.Resources, expression)
	if err != nil {
...
func configureCLIEvents(block *cliRun, deps core.BlockDeps, cfg cliRunSettings) error {
	if cfg.Events == nil {
		if len(cfg.Emit) > 0 {
	...
	flow, err := flows.Branch(core.BranchEvents, *cfg.Events)
	if err != nil {
		return fmt.Errorf("cli-run events: %w", err)
	}
	kinds, err := observe.Kinds(blockKindCLIRun, cliEventKinds, cfg.Emit)
	if err != nil {
		return err
	}
	block.events = observe.New(flow, kinds, blockKindCLIRun)
	return nil
}

// blockLabel names the block in a log line or an error: its name, else its type.
func blockLabel(name string) string {
	if name != "" {
		return name
	}
	return blockKindCLIRun
}
...
func (c *cliRun) Process(ctx context.Context, msg *types.Message) (*types.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cli-run %s: %w", blockLabel(c.name), err)
	}
	...
	if c.onExit == onExitFail && result.exitCode != 0 {
		return nil, fmt.Errorf("cli-run %s: %q exited %d",
			blockLabel(c.name), program, result.exitCode)
	}
	return msg, nil
}
