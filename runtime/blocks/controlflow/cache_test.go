...
func buildCacheScope(t *testing.T, reg *core.BlockRegistry, key, ttl string) *cacheScope {
	...
	proc, err := tryBuildBlock(reg, core.BlockDeps{}, types.BlockConfig{
		...
		Settings: types.Settings{"key": key, "ttl": ttl, "body": &body}})
	if err != nil {
...
func buildStoppingCacheScope(t *testing.T, counter *int) *cacheScope {
	...
	proc, err := tryBuildBlock(reg, core.BlockDeps{}, types.BlockConfig{
		Type:     blockKindCacheScope,
		Settings: types.Settings{"key": `"k"`, "ttl": "1m", "body": &body}})
	if err != nil {
		t.Fatalf("build cache-scope: %v", err)
	}
	cs, ok := proc.(*cacheScope)
...
func TestCacheScopeBuildValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no key", cfg: types.BlockConfig{Type: blockKindCacheScope, Settings: types.Settings{"body": &body}}},
		{
			name: "bad key expr",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := tryBuildBlock(reg, core.BlockDeps{}, tt.cfg); err == nil {
