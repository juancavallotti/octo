...
// splitSettings configures the split block.
...
// newSplit builds a split from its settings.
//
//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newSplit(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	var cfg splitSettings
	...
	if err := s.resolveMode(deps, cfg); err != nil {
		return nil, err
	}
	...
	response, err := buildTakeoverResponse(blockKindSplit, deps, cfg.BuildResponse)
	if err != nil {
		return nil, err
	}
...
func (s *split) resolveMode(deps core.BlockDeps, cfg splitSettings) error {
	switch s.mode {
