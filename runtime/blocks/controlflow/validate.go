...
func (v *validateBlock) Process(ctx context.Context, msg *types.Message) (*types.Message, error) {
	var failures []any
	for i := range v.rules {
		ok, err := expr.EvalCondition(v.rules[i].program, msg, v.env)
		if err != nil {
...
// validateSettings configures the validate composite.
type validateSettings struct {
	...
	// env, now.
	Rules []validateRuleSettings `json:"rules" octo:"label=Rules,type=rule-list,required"`
	...
	OnReject *types.FlowConfig `json:"onReject" octo:"label=On reject,type=flow"`
}

// validateRuleSettings is one assertion of a validate block: a boolean CEL Expr
// that must hold, and the Message surfaced (in vars.validationErrors and the
// built-in response) when it does not.
type validateRuleSettings struct {
	Expr    string `json:"expr"`
	Message string `json:"message,omitempty"`
}

//nolint:ireturn // a BlockFactory returns the MessageProcessor interface
func newValidate(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	var cfg validateSettings
	...
	for i := range cfg.Rules {
		...
		program, err := expr.CompileMessage(deps.Resources, cfg.Rules[i].Expr)
		if err != nil {
			return nil, fmt.Errorf("validate rule %d: %w", i, err)
		}
	...
	onReject, err := buildOnReject(deps, cfg.OnReject)
	if err != nil {
		return nil, err
	}
