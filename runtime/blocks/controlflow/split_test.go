...
func TestSplitExpressionModeFansOutWithGroupVars(t *testing.T) {
	block, cont := buildSplit(t, types.BlockConfig{Settings: types.Settings{"items": "body.lines"}})
...
func TestSplitElementsDoNotAliasTheParentBody(t *testing.T) {
	block, cont := buildSplit(t, types.BlockConfig{Settings: types.Settings{"items": "body.rows"}})
...
func TestSplitTokenizingModes(t *testing.T) {
	tests := []struct {
		...
		{
			name: "lines",
			cfg:  types.BlockConfig{Settings: types.Settings{"mode": splitModeLines}},
			body: "one\ntwo\nthree",
		...
		{
			name: "delimiter",
			cfg:  types.BlockConfig{Settings: types.Settings{"mode": splitModeDelimiter, "delimiter": ","}},
			body: "a,b,c",
			want: []string{"a", "b", "c"},
		},
		{
			name: "chunk",
			cfg:  types.BlockConfig{Settings: types.Settings{"mode": splitModeChunk, "chunkSize": 2}},
			body: "abcde",
			want: []string{"ab", "cd", "e"},
		},
		{
...
func TestSplitStreamingRevealsSizeOnTheFinalElement(t *testing.T) {
	block, cont := buildSplit(t, types.BlockConfig{Settings: types.Settings{"mode": splitModeLines}})
...
func TestSplitReportsTheRealTotalToBuildResponse(t *testing.T) {
	...
	proc, err := tryBuildBlock(reg, core.BlockDeps{}, types.BlockConfig{
		Type:     blockKindSplit,
		Settings: types.Settings{"mode": splitModeLines, "buildResponse": &response}})
	if err != nil {
		t.Fatalf("build split: %v", err)
	}
	block, _ := proc.(*split)
...
func TestSplitDispatchErrorPolicy(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			block, cont := buildSplit(t, types.BlockConfig{Settings: types.Settings{"items": "body.lines", "onError": tt.onError}})
...
func TestSplitRejectsBadConfig(t *testing.T) {
	tests := []struct {
		...
		{name: "expression mode without items", cfg: types.BlockConfig{Settings: types.Settings{"mode": splitModeExpression}}},
		{name: "unknown mode", cfg: types.BlockConfig{Settings: types.Settings{"mode": "sideways"}}},
		{name: "delimiter mode without a delimiter", cfg: types.BlockConfig{Settings: types.Settings{"mode": splitModeDelimiter}}},
		{name: "chunk mode without a size", cfg: types.BlockConfig{Settings: types.Settings{"mode": splitModeChunk}}},
		{name: "chunk mode with a negative size", cfg: types.BlockConfig{Settings: types.Settings{"mode": splitModeChunk, "chunkSize": -1}}},
		{name: "unknown onError", cfg: types.BlockConfig{Settings: types.Settings{"items": "body.a", "onError": "explode"}}},
		{name: "foreign slot", cfg: types.BlockConfig{Settings: types.Settings{"items": "body.a", "condition": "true"}}},
	}
...
// nestedBuild builds cfg inside another block's slot — an if's then chain — which
// is where a block that continues the flow has no rest-of-the-flow to continue into.
func nestedBuild(cfg types.BlockConfig) error {
	_, err := tryBuildBlock(testRegistry(), core.BlockDeps{}, types.BlockConfig{
		Type: "if",
		Settings: types.Settings{
			"condition": "true",
			"then":      types.FlowConfig{Process: []types.BlockConfig{cfg}},
		},
	})
	return err
}
...
func TestSplitRejectsBeingNested(t *testing.T) {
	...
	// vanish at runtime.
	err := nestedBuild(types.BlockConfig{Type: blockKindSplit, Settings: types.Settings{"items": "body.a"}})
	if err == nil {
...
func TestSplitRequiresATextBodyToTokenize(t *testing.T) {
	block, _ := buildSplit(t, types.BlockConfig{Settings: types.Settings{"mode": splitModeLines}})
...
func TestSplitRejectsANonArrayItemsResult(t *testing.T) {
	block, _ := buildSplit(t, types.BlockConfig{Settings: types.Settings{"items": "body.count"}})
