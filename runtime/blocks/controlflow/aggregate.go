...
// aggregateSettings configures the aggregate block.
type aggregateSettings struct {
	...
	// it completes.
	BuildResponse *types.FlowConfig `json:"buildResponse" octo:"label=Build response,type=flow"`
}

// newAggregate builds an aggregate from its settings.
//
...
func newAggregate(raw types.Settings, deps core.BlockDeps) (core.MessageProcessor, error) {
	...
	if err := a.compileExpressions(deps, cfg); err != nil {
		return nil, err
	}
	...
	var err error
	if a.buildRespone, err = buildTakeoverResponse(blockKindAggregate, deps, cfg.BuildResponse); err != nil {
		return nil, err
	}
	return a, nil
}
...
func (a *aggregate) compileExpressions(deps core.BlockDeps, cfg aggregateSettings) error {
	correlation := cfg.Correlation
	if correlation == "" {
		correlation = defaultCorrelation
	}
	var err error
	if a.correlation, err = expr.CompileMessage(deps.Resources, correlation); err != nil {
	...
	if a.completionSize, err = expr.CompileMessage(deps.Resources, completionSize); err != nil {
		return fmt.Errorf("aggregate completionSize: %w", err)
	}

	if cfg.CompletionExpression != "" {
		if a.completionExpr, err = expr.CompileMessage(deps.Resources, cfg.CompletionExpression); err != nil {
			return fmt.Errorf("aggregate completionExpression: %w", err)
		}
	}
	return a.resolveStrategy(deps, cfg)
}
...
func (a *aggregate) resolveStrategy(deps core.BlockDeps, cfg aggregateSettings) error {
	switch cfg.Strategy {
	...
	case aggregateStrategyExpression:
		...
		fold, err := expr.CompileMessage(deps.Resources, cfg.Expression)
		if err != nil {
...
func (a *aggregate) resolveCompletion(cfg aggregateSettings) error {
...
func (a *aggregate) applyMessage(msg *types.Message, state *groupState) (string, bool, error) {
	...
	if a.completionExpr != nil {
		done, err := expr.EvalCondition(a.completionExpr, a.withGroup(msg, state), a.env)
		if err != nil {
