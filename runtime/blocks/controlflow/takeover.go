...
//
// It is also where a takeover block learns whether it may be built at all: a
// block that continues the flow needs a root chain to continue into, so one
// inside another block's slot is refused here, by name, while the author is
// still looking.
//
//nolint:ireturn // the seam returns the MessageProcessor interface
func buildTakeoverResponse(
	kind string, deps core.BlockDeps, cfg *types.FlowConfig,
	...
	if cfg == nil || len(cfg.Process) == 0 {
		return nil, nil //nolint:nilnil // an absent optional slot is not an error
	}
	flow, err := flows.Branch(core.BranchBuildResponse, *cfg)
	if err != nil {
...
// twice.
func applyTakeover(
	ctx context.Context, msg *types.Message, buildResponse core.MessageProcessor,
) (*types.Message, error) {
