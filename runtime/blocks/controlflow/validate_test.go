...
func TestValidatePassesWhenAllRulesHold(t *testing.T) {
	proc := buildValidate(t, types.BlockConfig{
		Type: "validate",
		Settings: types.Settings{"rules": []map[string]any{
...
func TestValidateRejectStatusOverride(t *testing.T) {
	proc := buildValidate(t, types.BlockConfig{
		Type:     "validate",
		Settings: types.Settings{"rejectStatus": 400, "rules": []map[string]any{{"expr": "false", "message": "always fails"}}}})
...
func TestValidateOnRejectSubflow(t *testing.T) {
	...
	proc := buildValidateWithReg(t, reg, types.BlockConfig{
		Type:     "validate",
		Settings: types.Settings{"rules": []map[string]any{{"expr": "false", "message": "nope"}}, "onReject": &types.FlowConfig{Process: []types.BlockConfig{{Type: "respond-418"}}}}})
...
func TestValidateRequiresRules(t *testing.T) {
	reg := testRegistry()
	_, err := tryBuildBlock(reg, core.BlockDeps{}, types.BlockConfig{Type: "validate"})
	if err == nil {
