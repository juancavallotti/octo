...
//
// It is a package rather than a few lines in main because two modules need the
// same three decisions — how the URL is parsed, how long the first connection is
// given, and what a failure means — and because the aggregator and the
// orchestrator disagree about the last one. The aggregator cannot work without
// Redis; the orchestrator only reports on it. Both use Open, and each decides
