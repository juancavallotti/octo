...
func TestDocumentFiltersByTag(t *testing.T) {
	rec := serve(t, "/openapi.json?tag=traces")
	if rec.Code != http.StatusOK {
		t.Fatalf("want 200, got %d: %s", rec.Code, rec.Body.String())
	}
	...
	if len(paths) == 0 {
		t.Fatal("want the trace routes kept")
	}
	for path := range paths {
		if !strings.HasPrefix(path, "/traces") {
			t.Errorf("want only trace routes, got %q", path)
		}
	}
}
...
func TestFilteredDocumentCarriesLessThanTheFullOne(t *testing.T) {
	...
	keptPaths, keptSchemas := decode(t, serve(t, "/openapi.json?tag=logs").Body.Bytes())
...
func TestDocumentRejectsBothFiltersAtOnce(t *testing.T) {
	rec := serve(t, "/openapi.json?tag=logs&path=/traces")
