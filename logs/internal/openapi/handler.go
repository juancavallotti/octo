...
// true whether or not that API's storage is wired, and a service still waiting on
// Postgres is exactly when someone wants to read it.
...
func (h *Handler) document(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("openapi handler", "err", err)
	...
	if _, err := w.Write(out); err != nil {
		slog.Error("openapi: write document", "err", err)
	}
}
